...
export default function Bubble({ role = "bot", children, ts }) {
  ...
  return (
    <div className={`flex items-end gap-2 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser && <img src={botAvatar} alt="Bot avatar" className="h-8 w-8 rounded-full object-cover" />}
      ...
      >
        {children}
        {!!time && (
          <div className={`mt-1 text-[10px] ${isUser ? "text-white/70" : "text-gray-500 dark:text-zinc-400"}`}>
            {time}
          </div>
        )}
      </div>

      {isUser && <img src={userAvatar} alt="User avatar" className="h-8 w-8 rounded-full object-cover" />}
    </div>
  );
}
